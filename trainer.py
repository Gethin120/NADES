...
class Trainer:
    ...
    def loop(self, dataloader: Iterable, stage: Stage, prefix: str) -> Metrics:
        ...
        if stage == "test":
            ...
            # return {f'{prefix}{stage}/{key}': round(value,4) for key, value in metrics.items()}
...
class Trainer_multi_gpu:
    ...
    def fit(
        ...
        # 关键：直接使用传入的模型（可能是DataParallel包装的）
        self.model = model
        self.optimizer = optimizer
        self.global_weights = global_weights
        monitor_key = f"{prefix}{self.monitor}"
        ...
        # 检查是否是DataParallel模型
        is_multi_gpu = isinstance(self.model, nn.DataParallel)
        if is_multi_gpu:
            self.logger.info(
                f"检测到DataParallel模型，使用GPU: {self.model.device_ids}"
            )
    ...
    def loop(self, dataloader: Iterable, stage: Stage, prefix: str) -> Metrics:
        ...
        y_list = []
        for batch in dataloader:
            logits, y = self.step(batch, stage, prefix)
            logits_list.append(logits)
            y_list.append(y)
        logits = torch.cat(logits_list, dim=0)
        y = torch.cat(y_list, dim=0).cpu()
        if stage == "test":
            ...
            auc = roc_auc_score(y, probs)
            loss = F.cross_entropy(logits.cpu(), y)
            metrics = {"auc": auc, "loss": loss.item()}
            return {f"{prefix}{stage}/{key}": value for key, value in metrics.items()}
    ...
    def step(self, batch, stage: Stage, prefix: str) -> Metrics:
        ...
        # 关键：正确处理DataParallel模型
        if isinstance(self.model, nn.DataParallel):
            # 检查是否使用了DataParallelWrapper
            if hasattr(self.model.module, "module"):
                # 这是DataParallelWrapper包装的模型
                logits = self.model(batch)  # DataParallel自动处理多GPU分发
                ...
                # 计算损失
                loss = F.cross_entropy(logits_masked, y, weight=self.global_weights)
            else:
                # 直接DataParallel包装的模型
                if hasattr(self.model.module, "forward_parallel"):
                    # 使用DataParallel的并行前向传播
                    logits = self.model(batch)  # 这里会自动分发到多GPU
                    ...
                    # 回退到module.step方法
                    loss, logits_masked, y = self.model.module.step(
                        batch, stage=stage, global_weights=self.global_weights
                    )
        else:
            # 对于普通模型，直接调用step
