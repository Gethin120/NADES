...
class NodeDataLoader:
    ...
    def __iter__(self) -> Iterator[Data]:
        ...
        if self.shuffle and not self.poisson_sampling:
            # perm = torch.randperm(self.num_nodes, device=self.device)#2025年11.3修改，因为self.node_indices在cpu上，搬动较小
        ...
        for i in range(0, self.num_nodes, self.batch_size):
            ...
            if self.poisson_sampling:
                ...
                sample_mask = (
                    torch.rand(self.num_nodes, device=self.device) < sampling_prob
                )
            ...
            if self.hops is None:
                batch_mask = torch.zeros(
                    self.data.num_nodes, device=self.device, dtype=torch.bool
                )
                ...
                subset, batch_edge_index, mapping, _ = k_hop_subgraph(
                    ...
                    num_hops=self.hops,
                    edge_index=self.data.edge_index,  # 这里改过原来是self.edge_index
                    relabel_nodes=True,
                    num_nodes=self.data.num_nodes,
                )

                batch_mask = torch.zeros(
                    subset.size(0), device=self.device, dtype=torch.bool
                )
